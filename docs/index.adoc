...
The core of this document is about how to describe and implement XCP support for FMUs, either by an FMU internal XCP slave implementation or an external XCP slave.
...
_[Note that the FMU must set at least one of the attributes `containsXcpSlave` or `canUseExternalXcpService` to make use of this layered standard.]_
...
If the FMU announces that it makes use of the intermediate update callback function (attribute `providesIntermediateUpdate = true` of the Co-Simulation element), the importer may also create another event for the intermediate update raster and call the XCP service in each `fmi3IntermediateUpdateCallback` with the event channel number of this event.
...
* Page Handling (external)
* Event synchronous measurement (external)
* Multiple User DLLs
...
#ToDo# Use key word "automatic address update" according to XCP standard