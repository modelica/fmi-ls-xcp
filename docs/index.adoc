...
Therefore, if the XCP slave is embedded in the virtual ECU, the FMU should expose the following variables in the `modelDescription.xml` file, which are used to configure the embedded XCP slave (see <<starting-internal-xcp-service>>).

Enable XCP on TCP:: Determines whether the TCP instance of the XCP service provided
+
[cols="1h,4"]
|===
|Annotation          |`org.fmi-standard.fmi-ls-xcp.EnableXcpOnTcpIp`
|Type                |`Boolean` 
...
Enable XCP on UDP:: Determines whether the UDP instance of the XCP service provided by the FMU shall be started.
+
[cols="1h,4"]
|===
|Annotation          |`org.fmi-standard.fmi-ls-xcp.EnableXcpOnUdpIp`
|Type                |`Boolean` 
...
[cols="1h,4"]
|===
|Annotation          |`org.fmi-standard.fmi-ls-xcp.UdpListenPortNumber` 
|Type                |`UInt16` / `Integer` 
|Example Start Value |In an agreed range of ports, e.g., `32768` to `39999`.
|Requirement Type    |Should be supplied, if the FMU provides an XCP service using UDP.
|If not present      |Defaults to the protocol data values found in the relevant A2L file.
...
TCP listen IP address:: IP address where the XCP slave listens for XCP protocol commands.
+
[cols="1h,4"]
|===
...
UDP listen IP address:: IP address where the XCP slave listens for XCP protocol commands.
+
[cols="1h,4"]
|===
|Annotation          |`org.fmi-standard.fmi-ls-xcp.UdpListenIpAddress`
|Type                |`String` 
|Example Start Value |`0.0.0.0`.
|Requirement Type    |Should be supplied, if the FMU provides an XCP service using UDP.
|If not present      |Defaults to the protocol data values found in the relevant A2L file.
...
[source, xml]
----
include::examples/annotations.xml[tags=ConfigurationVariables]
----
