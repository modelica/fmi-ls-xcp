...
#ToDo# Add "How to read this document" section
...
There are alternative implementations depending on the use case and data availability:
 ...
 . An external XCP slave implementation accesses the memory of the virtual ECU to expose the XCP protocol to the MCD tool.
   ...
   `fmi3IntermediateUpdate` calls or the clocks mechanism could be used to synchronize DAQ lists.
...
#ToDo# Structural Parameter as a part of the manifest file instead of the modelDescription???
...
The definitions that are specific to the particular implementation are defined in <<FMU with Integrated XCP Service>> and <<External XCP Service>>.
...
For more information about how to configure and handle the XCP server using the different alternatives (internal/external XCP server), see <<FMU with Integrated XCP Service>> and <<External XCP Service>>, respectively.
...
Sometimes it is necessary for the FMU importer to override the default IP address and/or port number which was assigned by the FMU exporter in the A2L file (see <<XCP Protocol Settings>>). Furthermore, if the FMU additionally supports an external XCP service, it must be possible to deactivate the internal implementation.

Therefore, if the XCP slave is embedded in the virtual ECU, the FMU shall expose three structural parameters which are used to configure the embedded XCP slave (see <<internal-xcp-service-operation>>):
...
If the simulator puts the FMU in `Configuration Mode` and sets the structural parameters `org.fmi-standard.fmi-ls-xcp.IPAddress` and `org.fmi-standard.fmi-ls-xcp.PortNumber`, the XCP slave shall use those parameters to set up the communication connection for the XCP protocol.
...
In this case it is the choice of the importer whether to use the FMU internal XCP slave or its own implementation. See <<internal-xcp-service-operation>> on how to deactivate the internal XCP service.

Although the XCP behavior for an internal and an external service is the same from the perspective of the XCP master, the operation of the external XCP service differs from the internal in some points that are described in this chapter.

_[Figure 3 (#ToDo# Replace with link) shows a design where the XCP slave is implemented by the importer outside of the FMU._
...
When an external XCP service is used the importer is the one who has to make XCP service calls but it does not have the control over the different XCP events, because only the FMU knows when an event channel is active.
This means, by default, the importer can only make XCP calls at certain points in time disregarding specific XCP events, e.g. at communication points in Co-Simulation.

To allow the importer to make event-synchronous XCP calls the FMU must forward the XCP events to the importer.

The FMU announces that it signals XCP events to the importer by adding one clock variable for each event channel to the `modelDescription.xml` file:

.XCP Event Clocks
[#figure-xcp-event-channel-variable]
----
    org.fmi-standard.fmi-ls-xcp.Event<event-channel-name>
        Description:          "Clock that signals a specific XCP event to the importer."
        Type:                 Clock
        Causality:            <e.g. local>
        Variability:          discrete
        IntervalVariability:  <e.g. triggered>
----
...
#ToDo# Check FMI interface independency +

#ToDo# Was ist spezifisch für XCP Services, die den Host Stack benutzen? +
#ToDo# Variablen überall umbenennen +
#ToDo# Auch andere Gründe für Abschalten des internen XCP Service nennen, als nur dass man seinen eigenen verwenden möchte. +