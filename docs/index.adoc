...
The XCP slave implementation is responsible for translating the addresses from the A2L file to the physical addresses needed to access the variable values.
...
The XCP slave must be invoked in the FMU code according to the defined measurement raster and in the background as described in <<XCP Slave Implementation>>.
