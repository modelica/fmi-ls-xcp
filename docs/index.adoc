...
There are alternative implementations depending on the use case and data availability:
 ...
 . The FMU implements one or more XCP slaves which provide access to measurement and calibration variables of the virtual ECU and handles the communication protocol with the XCP master in the MCD tool.
   The necessary information for an MCD tool is given in one or more description files which follow the https://www.asam.net/standards/detail/mcd-2-mc/[ASAM MCD-2 MC] standard (aka A2L, also <<ASAP2>>) and customarily carries the file extension `.a2l`.
 ...
 . An external XCP slave implementation accesses the memory of the virtual ECU to expose the XCP protocol to the MCD tool.
   In this case, the importer needs to provide such an XCP slave implementation using the A2L description file(s) provided by the FMU.
...
This layered standard defines additional elements that carry relevant capability flags in the layered standard manifest file.
...
The root element must contain at least one, and can contain more than one `Access` elements.
Each such element specifies one set of access information for calibration using one underlying A2L file.
Multiple elements might be provided if there are different XCP slaves embedded in the FMU, or there are other reasons why multiple A2L files have to be provided.

Each `Access` element has the following attributes:

<<table-schema-fmi-ls-xcp-attributes>> shows the attributes of the root element of `fmi-ls-manifest.xml`.

.Access Element Attribute Details.
[[table-schema-fmi-ls-xcp-access-attributes]]
[cols="1,1,1,2",options="header"]
|====
|Attribute
|Namespace
...
|`identifier`
|
| String that would be a valid modelIdentifier
| This identifier will be used to generate the root name of the A2L file to be used for this calibration access mechanism.
...
|`variableEnableXCPOverTCP`
|
| Valid FMU variable name
| Provides the name of the variable that determines whether the TCP instance of the XCP service provided by the FMU shall be started.
See <<configure-internal-xcp-service-settings>>.

|`variableEnableXCPOverUDP`
|
| Valid FMU variable name
| Provides the name of the variable that determines whether the UDP instance of the XCP service provided by the FMU shall be started.
See <<configure-internal-xcp-service-settings>>.

|`variableTCPListenPortNumber`
|
| Valid FMU variable name
| Provides the name of the variable that controls the TCP port number where the XCP slave listens for XCP protocol commands.
See <<configure-internal-xcp-service-settings>>.

|`variableUDPListenPortNumber`
|
| Valid FMU variable name
| Provides the name of the variable that controls the UDP port number where the XCP slave listens for XCP protocol commands.
See <<configure-internal-xcp-service-settings>>.

|`variableTCPListenIPAddress`
|
| Valid FMU variable name
| Provides the name of the variable that controls the IP address where the XCP slave listens for XCP protocol commands over TCP.
See <<configure-internal-xcp-service-settings>>.

|`variableUDPListenIPAddress`
|
| Valid FMU variable name
| Provides the name of the variable that controls the IP address where the XCP slave listens for XCP protocol commands over UDP.
See <<configure-internal-xcp-service-settings>>.

|====

The attributes `identifier`, `containsXCPService`, and `supportsDirectMemoryAccessViaA2L` are required, all other attributes of `Access` are optional.
The FMU must provide a valid `identifier` attribute and must set at least one of the attributes `containsXCPService` or `supportsDirectMemoryAccessViaA2L` to `true` to comply with this layered standard.
...
A2L description files and other files needed for XCP support are placed in `/extra/org.fmi-standard.fmi-ls-xcp`.
...
extra/org.fmi-standard.fmi-ls-xcp  // Contains files related to XCP/A2L
   ...
   <identifier>.a2l                // Variable description of a source code FMU (optional)
   x86_64-windows
      <identifier>.a2l             // Variable description for Windows on Intel 64-bit (optional)
   x86-linux
      <identifier>.a2l             // Variable description for Linux on Intel 32-bit (optional)
   aarch32-linux
      <identifier>.a2l             // Variable description for Linux on ARM 32-bit (optional)
   x86_64-darwin
      <identifier>.a2l             // Variable description for macOS (optional)
----

The `<identifier>` above is the string of the corresponding `identifier` attribute of an `Access` element.
If multiple `Access` elements are present in the manifest, then multiple A2L description files will be present at each level of the directory structure, distinguised by their root name.
...
The root name of the A2L file shall be identical to the `identifier` attribute from the `Access` element of the manifest file and is case sensitive.
...
The variable that is provided for a capability is identified by providing the name of the variable in the corresponding capability attribute of the `Access` element of the manifest.

Enable XCP over TCP:: Determines whether the TCP instance of the XCP service provided by the FMU shall be started.
+
[cols="1h,4"]
|===
|Capability Attribute |`variableEnableXCPOverTCP`
|Type                 |`Boolean` 
...
[cols="1h,4"]
|===
|Capability Attribute |`variableEnableXCPOverUDP`
|Type                 |`Boolean` 
...
[cols="1h,4"]
|===
|Capability Attribute |`variableTCPListenPortNumber`
|Annotation           |`org.fmi-standard.fmi-ls-xcp.TcpListenPortNumber`
|Type                 |`UInt16` / `Integer` 
...
[cols="1h,4"]
|===
|Capability Attribute |`variableUDPListenPortNumber`
|Type                 |`UInt16` / `Integer` 
|Example Start Value  |In an agreed range of ports, e.g., `32768` to `39999`.
|Requirement Type     |Should be supplied, if the FMU provides an XCP service using UDP.
|If not present       |Defaults to the protocol data values found in the relevant A2L file.
...
[cols="1h,4"]
|===
|Capability Attribute |`variableTCPListenIPAddress`
|Annotation           |`org.fmi-standard.fmi-ls-xcp.TcpListenIpAddress`
|Type                 |`String` 
...
[cols="1h,4"]
|===
|Capability Attribute |`variableUDPListenIPAddress`
|Type                 |`String` 
|Example Start Value  |`0.0.0.0`.
|Requirement Type     |Should be supplied, if the FMU provides an XCP service using UDP.
|If not present       |Defaults to the protocol data values found in the relevant A2L file.
